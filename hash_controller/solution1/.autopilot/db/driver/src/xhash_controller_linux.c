...
typedef struct {
    ...
    XHash_controller_uio_map maps[ MAX_UIO_MAPS ];
} XHash_controller_uio_info;

/***************** Variable Definitions **************************************/
static XHash_controller_uio_info uio_info;
...
static int uio_info_read_name(XHash_controller_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/name", info->uio_num);
    return line_from_file(file, info->name);
}

static int uio_info_read_version(XHash_controller_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/version", info->uio_num);
    return line_from_file(file, info->version);
}

static int uio_info_read_map_addr(XHash_controller_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
static int uio_info_read_map_size(XHash_controller_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
int XHash_controller_Initialize(XHash_controller *InstancePtr, const char* InstanceName) {
	XHash_controller_uio_info *InfoPtr = &uio_info;
