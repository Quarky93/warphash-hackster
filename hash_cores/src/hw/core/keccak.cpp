...
static ap_uint<1600> pad_head(ap_uint<640> msg) {
#pragma HLS INLINE
    ap_uint<1600> tmp;
    tmp(1599, 1536) = msg(63, 0);
    tmp(1535, 1472) = 0x0100000000000000;
    tmp(1471, 1088) = 0x0;
    tmp(1087, 1024) = 0x0000000000000080;
    tmp(1023, 0) = 0x0;
    return tmp;
}

static ap_uint<1600> pad_tail(ap_uint<512> msg) {
#pragma HLS INLINE
    ap_uint<1600> tmp;
    tmp(1599, 1088) = msg;
...
#pragma HLS PIPELINE II=1
    ...
    tmp.vld = msg.vld;
    tmp.dest = msg.dest;
    tmp.data = keccak1600(to_state(pad_head(msg.data) ^ midstate));
    return tmp;
}
...
#pragma HLS PIPELINE II=1
    ...
    tmp.vld = msg.vld;
    tmp.dest = msg.dest;
    tmp.data = keccak1600(to_state(pad_tail(msg.data)));
    return tmp;
}
