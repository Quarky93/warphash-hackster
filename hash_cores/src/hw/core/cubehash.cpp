...
static hash_t finalize(state_t state) {
    xv ^= 1;
    for (int i = 0; i < 10; i++) {
...
#pragma HLS PIPELINE II=1
    ...
    state_t state;
    for (int i = 0; i < 32; i++) {
        state[i] = reverse_bytes_32(midstate(1023 - i * 32, 992 - i * 32));
    }
    state = add_input(state, pad_head(msg.data(127, 0)));
    state = transform(state);
    tmp.data = finalize(state);
    return tmp;
}
