...
#define HEAD_MODE false
#define TAIL_MODE true
...
static ap_uint<1024> pad_head(ap_uint<640> msg) {
#pragma HLS INLINE
    ap_uint<1024> tmp;
    tmp(1023, 384) = msg;
    tmp[383] = 0x1;
...
static ap_uint<1024> pad_tail(ap_uint<512> msg) {
#pragma HLS INLINE
    ap_uint<1024> tmp;
    tmp(1023, 512) = msg;
    tmp[511] = 0x1;
...
static ap_uint<512> blake(ap_uint<1024> padded_msg, bool mode) {
...
#pragma HLS PIPELINE II=1
    ...
    if (mode == HEAD_MODE) {
        init.v[12] = constants[4] ^ 640;
        init.v[13] = constants[5] ^ 640;
    } else {
...
#pragma HLS PIPELINE II=1
    ...
    tmp.vld = msg.vld;
    tmp.dest = msg.dest;
    tmp.data = blake(pad_head(msg.data), HEAD_MODE);
    return tmp;
}
...
#pragma HLS PIPELINE II=1
    ...
    tmp.vld = msg.vld;
    tmp.dest = msg.dest;
    tmp.data = blake(pad_tail(msg.data), TAIL_MODE);
    return tmp;
}
